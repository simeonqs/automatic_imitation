...
# Paths
path_data_neutral = 'analysis/data/data_neutral.csv'
path_model_latency = 'analysis/code/model_latency_ordinal.stan'
...
# Read data
dat = read.csv(path_data_neutral, na.strings = c('?', ''))
dat = dat[!is.na(dat$Individual),]
dat = dat[!is.na(dat$Latency),]
dat$Latency = as.numeric(dat$Latency)
dat$Latency[which(dat$Latency <= 0.1)] = 0.1
dat$Behaviour = dat$Behaviour |> str_to_title() |> str_remove_all(' ')
